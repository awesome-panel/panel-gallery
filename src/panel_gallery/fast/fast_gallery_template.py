...
class FastGalleryTemplate(Template):
    ...
    def export(self, target: str | Path = "index.html"):  # pylint: disable=arguments-differ
        """Exports the template to the target file

        Args:
            target: The path to the file to export to. Default is index.html
        """
        if isinstance(target, str):
            target = Path(str)  # type: ignore
        environment = jinja2.Environment(autoescape=True)
        template = environment.from_string(TEMPLATE)
        index_html = template.render(**self._render_variables)  # pylint: disable=protected-access
        target.write_text(index_html, encoding="utf8")
